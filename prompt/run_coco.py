...
CLASS_NAMES_FULL = [x.replace('(','').replace(')','').replace('_', ' ') for x in COCO_CLASSES]
...
def load_data_new(data_path : str, neg_thr = None, remap = False):
    ...
    if remap:
        ...
        neg_data = features[neg], (labels).new_ones(labels[neg].shape)*48, ious[neg]
    else:
...
if __name__ == "__main__":
    ...
    if mode == 'train':
        ...
        train_neg = sample(train_neg, len(train_neg[0])//int(neg_split), [48])
        ...
        freq = [x / len(train_data) * 48 for x in freq]
        freq = freq[:48]
    ...
    if mode == 'train':
        ...
        optimizer = SGD(model.parameters(), lr=2e-1)
        scheduler = build_lr_scheduler(optimizer, 6, 0, 0)

        for i in range(6):
            print(f"epoch{i+1}")
            train_epoch(model, optimizer, train_dl, freq, mode_train, num_classes=48)
            emb = get_embedding(model, CLASS_NAMES_FULL)
            print('val on base')
            test_embedding(emb, val_dl1)
            print('val on novel')
            ...
            test_neg(emb)
            if emb.shape[0] > 80:
                test_embedding(emb[:80], val_dl1)
                test_embedding(emb[:80], val_dl2)
                test_neg(emb[:80])
            scheduler.step()
        ...
        if ensemble_embedding.shape[0] > 80:
            print('80 only')
            ensemble_embedding = ensemble_embedding[:80]
            # test_embedding(ensemble_embedding[lvis_base_label_ids], train_dl)
            test_embedding(ensemble_embedding, val_dl1)
            test_embedding(ensemble_embedding, val_dl2)
            # test_embedding(ensemble_embedding, val_dlp)
        ...
        names = [f'checkpoints/gen6_ens/pos{i}epoch6.pth' for i in [5,6,7,8,9]]
        iou_embedding = [load_ens_embedding([name], norm = True)[:80] for name in names]
